...
function EFFECT:Init( data )
	...
	if ( GetConVar( "sfw_fx_particles" ):GetBool() ) then
		ParticleEffectAttach( "faton_dissolve", PATTACH_ABSORIGIN_FOLLOW, ent, -1 )
	else
		ParticleEffectAttach( "faton_dissolve_cheap", PATTACH_ABSORIGIN_FOLLOW, ent, -1 ) 
	end
...
function EFFECT:RenderOverlay( entity )
	...
	cam.Start3D( Pos, EyeAngles() )
		...
		render.SetColorModulation( 0, 255, 140 )
		render.SetBlend( fColFrac )
	
		render.MaterialOverride()
		entity:DrawModel()

		render.SetColorModulation( 1, 255, 140 )
		render.SetBlend( 1 )
	cam.End3D()
