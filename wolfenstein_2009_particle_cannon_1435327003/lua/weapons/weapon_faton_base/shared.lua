...
function SWEP:Think()
	...
	if IsValid(self.Owner) and self.Owner:Alive() and self.Owner:GetInfoNum("doom3_autoreload", 1) >= 1 and self:Clip1() <= 0 and self:Ammo1() > 0 and self:GetNextPrimaryFire() <= CurTime() and self.Primary.ClipSize > 0 then
		self:Reload()
		 BEAM_ON = 0
	end
	...
	if idle > 0 and CurTime() > idle then
		self:SetIdleDelay(0)
		self:SetState(DOOM3_STATE_IDLE)
		if self.IdleAmmoCheck then
			if IsValid(self) and self:GetState() == DOOM3_STATE_IDLE then
			...
			if self:GetState() == DOOM3_STATE_IDLE then
				self:SendWeaponAnim(ACT_VM_IDLE)
			end
		end
	end
...
local mat_crosshaircenter2 = surface.GetTextureID("doom3hud/crosshaircenter2")
local mat_crosshairlight = surface.GetTextureID("doom3hud/crosshairlight")
local mat_crosshairframe = surface.GetTextureID("doom3hud/crosshairframe")
...
function SWEP:DrawHUD()
	...
	if cvars.Number("doom3_crosshair") == 1 then
		...
		surface.DrawTexturedRect(x - 62, y - 62, 128, 128)
		
		surface.SetTexture(mat_crosshaircenter2)
		surface.SetDrawColor(40, 130, 110, 180)
		surface.DrawTexturedRect(x - 31, y - 31, 64, 64)
		
		surface.SetTexture(mat_crosshairlight)
		surface.SetDrawColor(col1)
		surface.DrawTexturedRect(x - 31, y - 31, 64, 64)
		
		surface.SetTexture(mat_crosshairframe)
		surface.SetDrawColor(col2)
		surface.DrawTexturedRect(x - 31, y - 31, 64, 64)
	elseif cvars.Number("doom3_crosshair") == 2 then	
