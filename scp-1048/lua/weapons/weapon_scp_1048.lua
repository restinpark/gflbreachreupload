		...
		if IsValid(ent) then
			if ent:IsPlayer() then return end
				...
				if self.Owner.AddAchievementProgress then
					self.Owner:AddAchievementProgress("BearBreaker", 1)
				end
					ent:Remove()
...
function SWEP:SecondaryAttack()
	...
	if self.Owner:GetAmmoCount(22) > 1 then
	...
	if self.Owner:GetAmmoCount(20) > 7 then
...
function SWEP:Reload()
	if self.Owner:GetAmmoCount(21) > 19 then
