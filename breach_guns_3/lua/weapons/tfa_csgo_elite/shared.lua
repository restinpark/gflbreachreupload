...
--Firing related
SWEP.Primary.Sound 			= Sound("TFA_CSGO_ELITE.1")				-- This is the sound of the weapon, when you shoot.
...
SWEP.IronSightsMoveSpeed  = 0.80 --Multiply the player's movespeed by this when sighting.
...
SWEP.ViewModel			= "models/weapons/tfa_csgo/c_pist_elite.mdl" --Viewmodel path
...
SWEP.DefaultHoldType 				= "duel"		-- This is how others view you carrying the weapon. Options include:
...
-- SWEP.IronSightsPos = Vector(-5.2, 0, 2.25)
-- SWEP.IronSightsAng = Vector(-2.8, -5.3, 0)

SWEP.IronSightsPosLeft = Vector(5.16, 0, 2.04)
SWEP.IronSightsAngLeft = Vector(-2.28, 5.16, 0)

SWEP.IronSightsPosRight = Vector(-5.2, 0, 2.325)
SWEP.IronSightsAngRight = Vector(-3.02, -5.21, 0)

SWEP.IronSightsPos = SWEP.IronSightsPosLeft
SWEP.IronSightsAng = SWEP.IronSightsAngLeft
...
SWEP.BlowbackBoneMods = {
	["v_weapon.p250_slide"] = { scale = Vector(1, 1, 1), pos = Vector(0,0,-2), angle = Angle(0, 0, 0) },
...
SWEP.MuzzleFlashEffect = "tfa_csgo_muzzle_pistol_elite"

--[[EVENT TABLE]]--

SWEP.MagModel = Model("models/weapons/tfa_csgo/w_pist_elite_mag.mdl")
SWEP.EventTable = {
	[ACT_VM_RELOAD] = {
		{ time = 14 / 30, type = "lua", value = function(wep,vm) wep:DropMag() end, client = true, server = true },
...
SWEP.RTCode = nil --[[ function( self ) --This is the function to draw onto your rendertarget
...
if CLIENT then
	function SWEP:ToggleISPos(num)
		local cycle = (num == "0" and not self.Akimbo_Inverted) or (num == "1" and self.Akimbo_Inverted)
		self.IronSightsPos = cycle and self.IronSightsPosRight or self.IronSightsPosLeft
		self.IronSightsAng = cycle and self.IronSightsAngRight or self.IronSightsAngLeft
		self:ClearStatCache()
	end
end
...
if SERVER then
	function SWEP:ToggleAkimbo(...)
		self:CallOnClient("ToggleISPos", self.AnimCycle)

		return BaseClass.ToggleAkimbo(self, ...)
	end
end