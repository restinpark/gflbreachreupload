SWEP.Gun					= ("tfa_csgo_cz75") --Make sure this is unique.  Specically, your folder name.  
...
SWEP.PrintName				= "CZ75-Auto"		-- Weapon name (Shown on HUD)	
...
--Firing related
SWEP.Primary.Sound 			= Sound("TFA_CSGO_CZ75.1")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_csgo/c_pist_cz_75.mdl" --Viewmodel path
...
SWEP.IronSightsPos = Vector(-2.81, 0, 1.15)
SWEP.IronSightsAng = Vector(-0.21, 0.025, 0)
...
SWEP.BlowbackBoneMods = {
	["v_weapon.cz_slide"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, -1), angle = Angle(0, 0, 0) }
}
...
SWEP.FrontMagAttached = false

SWEP.IsFirstDeploy = true

SWEP.MagModel = Model("models/weapons/tfa_csgo/w_pist_cz_75_mag.mdl")
SWEP.EventTable = {
	[ACT_VM_RELOAD] = {
		{ time = 16 / 30, type = "lua", value = function(wep,vm) wep:DropMag() end, client = true, server = true },
		{ ["time"] = 46	/ 30, ["type"] = "lua", ["value"] = function(wep,vm)
	...
	[ACT_VM_DRAW] = {
		...
		{ ["time"] = 0, ["type"] = "lua", ["value"] = function(wep,vm)
			wep:SetNW2Bool("FrontMagAttached", true)
		end, ["client"] = true, ["server"] = true },
	},
	[ACT_VM_DRAW_EMPTY] = {
		{ ["time"] = 0, ["type"] = "lua", ["value"] = function(wep,vm)
		...
		{ ["time"] = 0, ["type"] = "lua", ["value"] = function(wep,vm)
			wep:SetNW2Bool("FrontMagAttached", false)
		end, ["client"] = true, ["server"] = true },
	},
	[ACT_VM_RELOAD_EMPTY] = {
		{ time = 15 / 24, type = "lua", value = function(wep,vm) wep:DropMag() end, client = true, server = true }
	}
}

function SWEP:Initialize( ... )
	self.SetNW2Bool = self.SetNW2Bool or self.SetNWBool
	self:SetNW2Bool("FrontMagAttached", false)
...
function SWEP:ChooseDrawAnim()
	...
	if not self:GetNW2Bool("FrontMagAttached", false) and self:Ammo1() > 0 or self:GetNW2Bool("FrontMagAttached", true) and self:Ammo1() > 0 then
...
function SWEP:ChooseReloadAnim()
	...
	if self:GetNW2Bool("FrontMagAttached", true) then
...
-- function SWEP:FireAnimationEvent(pos, ang, event, options)
	...
	-- if event == 59 and self:GetNW2Bool("FrontMagAttached", false) and self:Ammo1() > 0 or self:GetNW2Bool("FrontMagAttached", true) and self:Ammo1() > 0 then
		-- self:SetBodyGroupVM(2, 0)
		-- self:SetNW2Bool("FrontMagAttached", true)
	-- else
		-- self:SetBodyGroupVM(2, 1)
    -- end
	
	-- if event == 58 and self:GetNW2Bool("FrontMagAttached", true) then
		-- self:SetBodyGroupVM(2, 1)
		-- self:SetNW2Bool("FrontMagAttached", false)
	-- end
