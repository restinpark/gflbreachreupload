SWEP.Gun					= ("tfa_csgo_zeus") --Make sure this is unique.  Specically, your folder name.  
...
SWEP.PrintName				= "Zeus x27"		-- Weapon name (Shown on HUD)	
SWEP.Slot = 1
SWEP.SlotPos = 10
...
--Firing related
SWEP.Primary.Sound 			= Sound("TFA_CSGO_Taser.1")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.HitSound 		= Sound("TFA_CSGO_Taser.Hit")				-- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.Ammo			= "csgo_taser"					-- What kind of ammo.  Options, besides custom, include pistol, 357, smg1, ar2, buckshot, slam, SniperPenetratedRound, and AirboatGun.  
...
SWEP.ViewModel			= "models/weapons/tfa_csgo/c_eq_taser.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_csgo/w_eq_taser.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(-4.85, 0, 2.28)
SWEP.IronSightsAng = Vector(0.61, -0.01, 0)
...
SWEP.BlowbackBoneMods = {
	["v_weapon.p250_slide"] = { scale = Vector(1, 1, 1), pos = Vector(0,0,-1), angle = Angle(0, 0, 0) },
	["v_weapon.p250_hammer"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 0, -37.385) }
}
...
SWEP.MuzzleFlashEffect = "tfa_csgo_muzzle_taser" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
SWEP.TracerParticleName = "weapon_tracers_taser"
...
function SWEP:PrimaryAttack()
	...
	if trace.Hit and IsValid(trace.Entity) and (self:GetPos():Distance(trace.HitPos) <= 300) and (trace.Entity:IsRagdoll() or trace.Entity:IsPlayer() or trace.Entity:IsNPC()) then
		self:EmitSound(self.Primary.HitSound)
	
		if CLIENT or game.SinglePlayer() and IsFirstTimePredicted() then
			util.Effect("tfa_tracer_csgo", fxdata)
		end
		...
		if trace.Entity then
			...
			local DamageValue = 0
			local dist = self:GetPos():Distance(trace.HitPos)
			DamageValue = DamageValue + (300 - dist)*2
			local damageMod = math.Clamp(DamageValue / 500,0,1)
			...
			self:PCFTracer(trace, trace.HitPos)
		if SERVER then
	...
	elseif trace.Hit and (self:GetPos():Distance(trace.HitPos) <= 300) then
	...
	self:SetNextPrimaryFire(CurTime() + self.Primary.RPM)
