...
--- Display of and interaction with ttt_traitor_button, code mostly from TTT
...
function TBHUD:CacheEnts()
   if IsValid(LocalPlayer()) and LocalPlayer():GetNClass() == ROLE_SCP079 then
      self.buttons = {}
      for _, ent in pairs(ents.FindByClass("br_079btn")) do
         if IsValid(ent) then
...
function TBHUD:PlayerIsFocused()
   return IsValid(LocalPlayer()) and LocalPlayer():GetNClass() == ROLE_SCP079 and IsValid(self.focus_ent)
end
...
local tbut_normal = surface.GetTextureID("tbut_hand_line")
local tbut_focus = surface.GetTextureID("tbut_hand_filled")
local size = 32
local mid  = size / 2
      ...
      for k, but in pairs(self.buttons) do
         ...
         if IsValid(focus_ent) then
            ...
            elseif focus_ent:GetDelay() == 0 then
               surface.DrawText("Infinite Uses")
            else
               surface.DrawText("Reusable in " .. tostring(focus_ent:GetDelay()))
            end

            y = y + 12
            surface.SetTextPos(x, y)
            surface.DrawText("+use to activate")
         end
      end
   end
end

hook.Add("PlayerBindPress", "PlayerBindPress_SCP079", function (ply, bind, pressed)
   if ply == LocalPlayer() and bind == "+use" and pressed then
      TBHUD:UseFocused()
   end
end)

hook.Add("HUDPaint", "DrawHUDForSCP079", function ()
   if LocalPlayer():GetNClass() == ROLE_SCP079 then
      --print('h')
