...
function ENT:TraitorUse(ply)
   if not (IsValid(ply) and ply:GetNClass() == ROLE_SCP079) then return false end
   ...
   --For breach, since the traitor buttons are created in the lua state, we also allow the user to define a UseFunc for the button
   if self.UseFunc then
     self.UseFunc(self, ply)
   end
...
local function TraitorUseCmd(ply, cmd, args)
   ...
   if IsValid(ply) and ply:GetNClass() == ROLE_SCP079 then
      local idx = tonumber(args[1])
      if idx then
         local ent = Entity(idx)
         if IsValid(ent) and ent:GetClass() == "br_079btn" and ent.TraitorUse then
            ent:TraitorUse(ply)
         end
      end
   end
