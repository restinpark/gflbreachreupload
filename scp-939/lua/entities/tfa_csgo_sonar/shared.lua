...
if SERVER then
	util.AddNetworkString("TFA_SONAR_EXPLODE_939")
end
...
function ENT:Think()
	if CLIENT then
		...
		local self = self:GetAttachment(attach)
...
if SERVER then
	...
	function ENT:Detonate()
		...
		if IsValid(self.Owner) then
			...
			net.Start("TFA_SONAR_EXPLODE_939")
			net.WriteTable(tab)
			net.Send(self.Owner)
		end
	...
	function ENT:PhysicsCollide(self, physObj)
		if not self:GetNWBool("Fused") then
			if self.HitEntity then
				if not (self.HitEntity:IsNPC() or self.HitEntity:IsPlayer()) then
					self:SetNWBool("Fused", true)

					timer.Simple(0, function()
						if IsValid(self) then
							self:Fuse(self.HitEntity)
						end
					end)
				end
			end
		end
		orient_angles(physObj,self)
	end
	
	function orient_angles(obj, self) --this function juts takes in the hitnormal of the collision and rotates the angles accordingly
		if self.HitNormal.z < -.5 then
			obj:SetAngles((self.HitNormal + Vector(0,90,0) ):Angle())
			return 
		end
		if self.HitNormal.z > .5 then
			obj:SetAngles((self.HitNormal + Vector(-90,0,0) ):Angle())
			return 
		end
		if self.HitNormal.y < -.5 then
			obj:SetAngles((self.HitNormal + Vector(0,0,90) ):Angle())
			return 
		end
		if self.HitNormal.y > .5 then
			obj:SetAngles((self.HitNormal + Vector(0,0,90) ):Angle())
...
if CLIENT then
	...
	net.Receive("TFA_SONAR_EXPLODE_939", TFA_SONAR_CREATE_HALOS)
