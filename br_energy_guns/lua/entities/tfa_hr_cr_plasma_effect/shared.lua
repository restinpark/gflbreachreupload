...
if CLIENT then
killicon.Add( "tfa_hr_cr_plasma_effect", "vgui/hud/halo_weapon_conclusion_rifle", color_white )
end
...
Glow:SetKeyValue("rendercolor","255 0 191")
	...
	local tr = util.TraceLine( trace )
		...
		if tr.Hit then
				local effectdata = EffectData()
					...
					util.Effect( "tfa_cr_hr_explode", effectdata )
					util.BlastDamage(self.Entity, self:OwnerGet(), tr.HitPos, 300, 170)
					util.Decal("Scorch", tr.HitPos + tr.HitNormal, tr.HitPos - tr.HitNormal)
					self.Entity:SetNWBool("smoke", false)
					
			self:Explosion()
			self.Entity:EmitSound( "pillar/elite_shot_explode_0"..math.random(1,6)..".wav", SNDLVL_110dB, 100, 0.8 )
			self.Entity:Remove()	
		end
	...
	pos = self:GetPos()
		for i=1, (1) do
			...
			if (particle) then
				...
				particle:SetEndSize( math.Rand( 130, 1 ) )
