...
self.Entity:SetColor (Color(0,0,0,255))       -- CHEESECAKE!    >:3           
...
self.Entity:SetNWBool("smoke", true)
	...
	local tr = util.TraceLine( trace )
		...
		if tr.Hit then
				local effectdata = EffectData()
					...
					effectdata:SetScale(0)			// Size of explosion
					effectdata:SetRadius(tr.MatType)		// What texture it hits
					effectdata:SetMagnitude(18)			// Length of explosion trails
					util.Effect( "", effectdata )
					util.BlastDamage(self.Entity, self:OwnerGet(), tr.HitPos, 60, 10)
 ...
 function ENT:Explosion()
	...
	local shake = ents.Create("env_shake")
		...
		shake:SetKeyValue("amplitude", "25")	// Power of the shake
		shake:SetKeyValue("radius", "1")		// Radius of the shake
		shake:SetKeyValue("duration", "2.5")	// Time of shake
		shake:SetKeyValue("frequency", "20")	// How har should the screenshake be
	...
	pos = self:GetPos()
		for i=1, (1) do
			...
			if (particle) then
				...
				particle:SetDieTime( math.Rand( 0, 0 ) )
