...
-- Fixing some items that do not equip
local itemsToFix = { "mikud" }
hook.Add("PlayerSpawn", "PS.ItemEquipPostSpawn", function(ply)
	-- Safe check
	if !IsValid(ply) then return end
	-- Get all items
	for _, id in ipairs(itemsToFix) do
		local item = PS.Items[id]
		-- Check if ply has the item
		if item && ply:PS_HasItem(id) then
			-- Do equip
