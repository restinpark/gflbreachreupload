...
PS.Config.CommunityName = "GFL Breach"
...
PS.Config.CalculateBuyPrice = function(ply, item)
	...
	if ply:IsUserGroup('vip') then return math.Round(item.Price * 0.85) end
	if ply:IsUserGroup('supporter') then return math.Round(item.Price * 0.90) end
	return item.Price
end
